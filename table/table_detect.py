...
import cv2
import numpy as np
from table.config import tableModelDetectPath
...
def table_detect(img,sc=(416,416),thresh=0.5,NMSthresh=0.3):
    ...
    inputBlob,fx,fy = utils.letterbox_image(img[...,::-1],(scale,scale))
    ...
    if len(boxes)>0:
        boxes, confidences = utils.nms_box(boxes, confidences, score_threshold=thresh, nms_threshold=NMSthresh)
...
if __name__=='__main__':
    import time
    p = 'D:/work/pycharm/doc_parsing/doc/pdf/rB8S22DDBx6ANSD0AA5watnx9X4002/img/page_001.png'
    img = cv2.imread(p)
    t =time.time()
    boxes,adBoxes,scores=table_detect(img, sc=(416, 416),thresh=0.5,NMSthresh=0.3)
    print(time.time()-t,boxes,adBoxes,scores)
    img = utils.rectangle(img,adBoxes)
    img.save(p)