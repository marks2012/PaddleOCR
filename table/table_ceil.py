...
class table:
   ...
   def img_degree(self):
       img,degree =utils.eval_angle(self.img,angleRange=[-15,15])
       self.img = img
       self.degree = degree
   ...
   def table_ceil(self):
       ...
       for i in range(n):
           ...
           tmp = utils.draw_lines(tmp,rowboxes+colboxes,color=255,lineW=2)
           labels = utils.measure.label(tmp<255,connectivity=2)  #8连通区域标记
           regions = utils.measure.regionprops(labels)
           ceilboxes= utils.minAreaRectbox(regions,False,tmp.shape[1],tmp.shape[0],True,True)
...
if __name__=='__main__':
    ...
    p = 'D:/work/pycharm/doc_parsing/doc/pdf/rB8S22DDBx6ANSD0AA5watnx9X4002/img/page_009.png'
    img = cv2.imread(p)
    t =time.time()
